...
for ifacont=1:size(bedge,1)
    ...
    if b==1
        ...
        flowrate(ifacont,1)= mobility(ifacont)*normcont*(parameter(1,1,ifacont)*(p(lef)-pinterp(ifacelef1))+parameter(1,2,ifacont)*(p(lef)-pinterp(ifacelef2)));
...
for iface=1:size(inedge,1)
    ...
    flowrate(iface+ size(bedge,1),1)=flowrate(iface+ size(bedge,1),1)*mobility(ifactual);
    %Attribute the flow rate to "flowresult"
    %On the left:
