...
function [coord,centelem,elem,esurn1,esurn2,nsurn1,nsurn2,bedge,inedge,...
    ...
    kmap,wells,klb,limiterflag,rowposit,hconv] = preprocessor 
...
[head,benchkey,pathstore,numcase,complemfile,phasekey,xyrz,r0,symaxe,...
    ...
    bcflag,numbcnode,bcnodepointer,courant,totaltime,numwell,well,hconv] = ...
    getdatafile;
...
elseif strcmp(keyfile,'n')  
    ...
    for i = 1:lengthgeodata
        ...
        elseif strcmp(rowgeo(1:5),'Point') && strcmp(keymesh,'n') %??????????????????????????????
            ...
            %value corrected
            fprintf(geometry,'Point(%u) = {%f, %f, %f, %f};\r\n',...
...
% pt4 = inedge(:,4) == 0;
% 
% sumpt3 = sum(inedge(:,3) == 0)
% sumpt4 = sum(inedge(:,4) == 0)
% 
% aa3 = ii(pt3)
% aa4 = ii(pt4)
% 
...
% vtx4 = inedge(aa4,1:2)
% 
% coord(inedge(aa3,1),1:2)
% coord(inedge(aa4,1),1:2)
...
elseif strcmp(keyperm,'y')
    ...
    if strcmp(hipermname,'0') == 0
        ...
        readperm = fopen(sprintf('%s',char(hipermname)));
        %"kmap" reads the file put inside work folder
...
%Initialize "bcflag"
getdata = textscan(readfile,'%f%f%f',numbc,'CommentStyle',{'//'});
%Attribute the data to "bcflag"
bcflag_aux = cell2mat(getdata);
%Fill "becflag"
bcflag = bcflag_aux(:,1:2);
    
%Verify if there exist a third value
pointisanum = isnan(bcflag_aux(:,3)) == 0;
%Initialize "hconv"
hconv = 0;
%Changes "hconv" if is the case.
if any(bcflag_aux)
    hconv = bcflag_aux(pointisanum,3);
end  %End of IF
...
%Initialize "theretri". Mark "1" if there some triangle in the domain.
theretri = 0;
...
%Initialize "klb" (Known Line Boundary)
klb = 0;
...
for ielem = 1:nelem
    ...
    %It ensures that "isquad" is "1" if there is any quadrangle in domain
    isquad = (isquad == 1) || (amountedges == 4);
    ...
    for inode = 1:amountedges
        ...
        %"inedge". Before that, some conditions must be satisfy.
            ...
            angcond = ((angbtweenedges/180) <= 0.999);
...
%Clear matrices
clear bedgnumrow getboundata bedgeaux ielem;
