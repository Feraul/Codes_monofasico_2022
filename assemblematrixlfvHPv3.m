function [M,I]=assemblematrixlfvHPv3(parameter,fonte,nflagface,weightDMP,wells,mobility,g)
...
if max(max(wells))~=0
    ...
    for iw = 1:size(wells,1)
        ...
        if wells(iw,2)==1            % injetor
            I(wells(iw,1))= wells(1,6)*elemarea(wells(iw,1));        % injeta um m3 de agua por dia (d)
    ...
    for ifacont=1:size(bedge,1)
        ...
        if bedge(ifacont,5)>200
            ...
            [M,I]=tratmentcontourlfvHP(ifacelef1,parameter,nflagface,...
                normcont,mobility,auxparameter1,auxmobility,lef,weightDMP,M,I,g);
            %-------------------------------------------------------------%
            % somando 2
...
%     M(rel,lef)=M(rel,lef)- mobility(ifactual)*ALL;
    ...
    if ifacerel1<size(bedge,1) || ifacerel1==size(bedge,1)
        if nflagface(ifacerel1,1)<200
            ...
            r=find(x==1);
            fluxoN=normcont*bcflag(r,2)+g(ifacerel1,1);
